...
export const testSubscription = async (pushToken: string) => {
    console.log("Testing subscriptions", pushToken)
    try {
        console.log(`https://us-central1-ipo-notify-4b5c1.cloudfunctions.net/testNotification?token=${pushToken}`)
        const response = await axios.get(`https://us-central1-ipo-notify-4b5c1.cloudfunctions.net/testNotification?token=${pushToken}`);
